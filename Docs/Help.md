...
Exception: An error is thrown when you send the wrong parameter, for example when you pass the wrong block height (too big block height) in BlockIdentifier
In the unit test, 2 example of the negative tests are given, 1 for sending with wrong state root hash, 1 for sending with wrong purse uref.
